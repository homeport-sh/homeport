# TanStack Start → single binary → homeport

A minimal TanStack Start app that compiles to one executable and deploys with
[homeport](../../). TanStack Start builds through **Nitro**; with the `bun`
preset the server output can be compiled into a single binary by
`bun build --compile`.

## The two things that make it work

1. **Three top-level Nitro options** — `vite.config.ts`:
   ```ts
   ...
   export default defineConfig({
     plugins: [
       ...
       // ...tanstackStart(), react(), etc.
     ],
   })
   ```
   This makes `vite build` emit a Bun-compatible server at
...
`homeport init` writes all of this automatically — it detects TanStack Start
from `package.json`.
...
```bash
bun install
homeport init                 # detects TanStack Start, writes homeport.yaml (already here)
...
For TanStack Start itself (routing, server functions, data loading), see the
